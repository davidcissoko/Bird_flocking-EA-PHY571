...
# --- Obstacles ---
OBSTACLES = [
    {"x": 20.0, "y": 20.0, "radius": 1.5, "influence_radius": 6.0}, {"x": 10.0, "y": 30.0, "radius": 2.0, "influence_radius": 5.0},
    {"x": 30.0, "y": 10.0, "radius": 2.5, "influence_radius": 7.0}
]
OBSTACLE_AVOIDANCE_MODE = "inverse_square"  # "inverse_square" ou "constant"
...
def get_params(config):
    ...
    if config == "fragmented":
        ...
        return dict(N=160, WIDTH=45.0, HEIGHT=45.0, dt=0.1,
                    ...
                    W_SEPARATION=2.0, W_ALIGNMENT=0.8, W_COHESION=0.4, W_OBSTACLE=3, T=2000)
    ...
    class BoidSimulation:
        ...
        def _update_kernel(self):
            ...
            for i in range(self.n):
                ...
                obs_repulsion = ti.Vector([0.0, 0.0])
                for k in range(num_obs):
                    dx = self.pos[i][0] - obs_x[k]
                    dy = self.pos[i][1] - obs_y[k]
                    d = ti.sqrt(dx*dx + dy*dy)
                    ...
                    if d < obs_influence[k] and d > obs_rad[k]:
                        # Interaction 1/r^2
                        force_mag = 1 / (d**2)
                        obs_repulsion += ti.Vector([dx / d, dy / d]) * force_mag
                
                obs_repulsion = self._limit_vec(obs_repulsion, MAX_FORCE * 2.0)
